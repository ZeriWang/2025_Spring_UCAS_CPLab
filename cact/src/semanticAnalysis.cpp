...
void SemanticAnalysis::enterCompUnit(CACTParser::CompUnitContext *ctx)
{
    ...
    sym_table.addFunc("print_char", CLASS_VOID, 1, 0);
    sym_table.addSymbol("_char_", CLASS_CHAR, TYPE_PARAM, 0, 0);
    sym_table.block_stack.pop_back();
    sym_table.cur_func = "$";
    ...
    sym_table.addFunc("get_char", CLASS_CHAR, 0, 0);
    sym_table.block_stack.pop_back();
    sym_table.cur_func = "$";
}
...
void SemanticAnalysis::enterFuncDef(CACTParser::FuncDefContext *ctx)
{
    ...
    else if (return_type_str == "char") {
        return_type = CLASS_CHAR;
    }
...
void SemanticAnalysis::enterConstDefVal(CACTParser::ConstDefValContext *ctx)
{
    ...
    else if (class_str == "char") {
        var_cls = CLASS_CHAR;
        if (ctx->constExp()->getStart()->getType() != CACTLexer::CharConst)
            throw std::runtime_error("line " + std::to_string(ctx->getStart()->getLine()) + ": error: invalid initializer of '" + var_name + "'\n");
        // 处理字符常量，去掉单引号并处理转义字符
        std::string char_str = literal_val.substr(1, literal_val.length() - 2);
        int char_val;
        if (char_str.length() == 1) {
            char_val = (int)char_str[0];
        } else if (char_str.length() == 2 && char_str[0] == '\\') {
            // 处理转义字符
            switch (char_str[1]) {
                case 'n': char_val = '\n'; break;
                ...
                case '\"': char_val = '\"'; break;
                case '?': char_val = '\?'; break;
                default: char_val = char_str[1]; break;
            }
        } else {
            throw std::runtime_error("line " + std::to_string(ctx->getStart()->getLine()) + ": error: invalid character constant\n");
        }
        value_str = std::to_string(char_val);
    }
    else{
    ...
    if (var->global) {
        ...
        for (auto &elem : ctx->constExp()) {
            ...
            if (var_cls == CLASS_INT && literal_type != CACTLexer::IntConst || var_cls == CLASS_BOOLEAN && literal_type != CACTLexer::BoolConst || \
                ...
                var_cls == CLASS_CHAR && literal_type != CACTLexer::CharConst)
                throw std::runtime_error("line " + std::to_string(ctx->getStart()->getLine()) + ": error: invalid initializer of '" + var_name + "'\n");
...
void SemanticAnalysis::exitPrimary(CACTParser::PrimaryContext *ctx)
{
    ...
    // number
        ...
        else if (num_cls == CACTLexer::CharConst) {
            ctx->cls = CLASS_CHAR;
            ctx->name = sym_table.gen_Temp_Var(ctx->getStart()->getLine(), CLASS_CHAR, sym_table);
            // 处理字符常量，去掉单引号并处理转义字符
            std::string char_str = literal_val.substr(1, literal_val.length() - 2);
            if (char_str.length() == 1) {
                char_val = (int)char_str[0];
